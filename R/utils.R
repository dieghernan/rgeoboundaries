...
country_to_iso3 <- function(country) {
  if (!is.null(country)) {
    ...
    ind <- nchar(country) >= 4
    if (any(ind)) {
      iso3 <- countryname(country[ind], destination = "iso3c")
...
assert_adm_lvl <- function(adm_lvl, dict = paste0("adm", 0:5)) {
  ...
  if (!cond) {
    stop(
      paste("Not a valid ADMIN level code! Use", paste(dict, collapse = ", ")),
      call. = FALSE
    )
  }
}
...
build_urls <- function(iso3, adm_lvl, release_type) {
  iso3 <- toupper(iso3)
  adm_lvl <- toupper(adm_lvl)
  l <- list(RELEASE_TYPE = release_type, ISO = iso3, ADM = adm_lvl)
  l <- drop_nulls(l)
  template_url <- paste0(baseurl(), create_query(names(l)))
...
xget_async <- function(urls) {
  reqs <- lapply(urls, function(url) {
    ...
    req$retry("get", times = 3, retry_only_on = c(500, 503), terminate_on = 404)
  })
  sleep <- 0.1
  ...
  if (any(cond)) {
    ...
    stop("One of the coutry names or iso3 codes is invalid", call. = FALSE)
  }
...
xget <- function(urls) {
  ...
  if (res$status_code >= 300) {
    stop("The country name or iso3 code is invalid!", call. = FALSE)
  }
...
.gb_meta <- function(
  ...
  if (length(urls) >= 2) {
    res <- xget_async(urls)
    res <- do.call(rbind, lapply(res, \(x) as.data.frame(fromJSON(x))))
  } else {
    res <- xget(urls)
  ...
  if (adm_lvl != "ALL") {
    if (!adm_lvl %in% res$boundaryType) {
      stop(paste0(adm_lvl, " not available!"), call. = FALSE)
    } else {
...
gb_metadata <- function(
  ...
  gb_meta(country = country, adm_lvl = adm_lvl, release_type = release_type)
}
...
get_adm_shp_link <- function(
  ...
  data.frame(
    path = path.expand(file.path(destfiles, files)),
