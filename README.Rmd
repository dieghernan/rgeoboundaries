...
The **rgeoboundaries** package provides an **R**-friendly interface to access
and work with the [**geoBoundaries**](https://www.geoboundaries.org/) dataset
(an open-license global database of administrative boundary polygons). Using
...
-   Work in an open-data context (geoBoundaries uses [CC
    BY-4.0](https://creativecommons.org/licenses/by/4.0/)) / open licences[^1])

[^1]: Individual data files in the geoBoundaries database are governed by the
    license or licenses identified within the metadata for each respective
    boundary. Users using individual boundary files from geoBoundaries should
...
```{r world, fig.cap="Map on the rgeoboundaries logo", crop=TRUE}
library(rgeoboundaries)
library(sf)
library(dplyr)
library(ggplot2)
...
This package is released under the [MIT](https://opensource.org/license/mit)
license. Note that the boundary data being accessed (via geoBoundaries) also
uses open licences; please check the specific dataset metadata for licensing
details.
...
-   Thanks to the **R** package community and all contributors to this package’s
    development.
-   If you use **rgeoboundaries** (and underlying geoBoundaries data) in your
...
toBibtex(citation("rgeoboundaries")[1])
```

## Legal

With respect to works on or made available through download from the this
...
particular purpose; nor represent that use of such works would not infringe
privately owned rights; nor assume any liability resulting from use of such
works; and shall in no way be liable for any costs, expenses, claims, or demands
arising out of use of such works.
